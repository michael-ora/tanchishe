...
class SnakeGame {
    ...
    generateFood() {
        ...
        do {
            const types = ['apple', 'hamster', 'rabbit', 'bird', 'chicken', 'duck', 'frog', 'mouse', 'snail', 'ladybug'];
            this.food = {
    ...
    draw() {
        ...
        this.snake.forEach((segment, index) => {
            ...
            if (index === 0) {
                ...
                // 计算尺寸缩放：只有最后三节逐渐变细
                let sizeScale = 1.0;
                const tailLength = 3; // 最后三节变细
                const tailStart = this.snake.length - tailLength;
                if (index >= tailStart && this.snake.length > tailLength) {
                    // 计算在尾部区间的位置（0 = 尾部起点, 1 = 最后一节）
                    const tailRatio = (index - tailStart) / (tailLength - 1);
                    // 从 1.0 缩小到 0.35
                    sizeScale = 1.0 - tailRatio * 0.65;
                }
                const bodySize = (size - padding * 2) * sizeScale;
                ...
                if (sizeScale > 0.5) {
                    ...
                    ctx.beginPath();
                    ctx.arc(sx + size / 2 - bodySize * 0.15, sy + size / 2 - bodySize * 0.15, bodySize * 0.08, 0, Math.PI * 2);
                    ctx.arc(sx + size / 2 + bodySize * 0.12, sy + size / 2 + bodySize * 0.1, bodySize * 0.06, 0, Math.PI * 2);
                    ctx.fill();
                }
            }
        });
